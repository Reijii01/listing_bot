...
from config import TOKEN2, DB_PATH, GROUP_CHAT_ID_FOR_REGION
...
def coins_command(update: Update, context: CallbackContext):
    if update.effective_chat.id != GROUP_CHAT_ID_FOR_REGION:
        return
...
def send_available_coins_keyboard(update: Update, context: CallbackContext):
    ...
    if query.message.chat.id != GROUP_CHAT_ID_FOR_REGION:
        return
...
def send_event(event):
    ...
    try:
        bot.send_message(chat_id=GROUP_CHAT_ID_FOR_REGION, text=text, parse_mode="HTML", disable_web_page_preview=True)
...
def notify_deleted_ticker(ticker):
    ...
    try:
        bot.send_message(chat_id=GROUP_CHAT_ID_FOR_REGION, text=text, parse_mode="HTML")
